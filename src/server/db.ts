...
export class FirestoreDatabaseStore {
  ...
  async createMember(data: Partial<ClubMember>): Promise<ClubMember> {
    ...
    const createdMember = await firestore.runTransaction(async (transaction) => {
      ...
      const member: ClubMember = {
        ...
        membershipFeeStartDate: data.membershipFeeStartDate || data.joinedDate || new Date().toISOString().split('T')[0],
        deactivationDate: data.deactivationDate || undefined,
        deactivationReason: data.deactivationReason || undefined,
    ...
    // Auto-generate fee/fine records according to member's fee start date
    await this.ensureMemberContributions(createdMember.id);
  ...
  async updateMember(id: string, updates: Partial<ClubMember>): Promise<ClubMember> {
    ...
    if (updates.status === 'inactive' && !updates.deactivationDate) {
      payload.deactivationDate = new Date().toISOString().split('T')[0];
    } else if (updates.status === 'active' && updates.status !== undefined) {
      payload.deactivationDate = null;
    }

    await docRef.set(payload, { merge: true });
    const snap = await docRef.get();
    const updated = snap.data() as ClubMember;

    // Auto-synchronize fee/fine records after member changes
    await this.ensureMemberContributions(id);
    return updated;
  }
  ...
  async updateIncomeRecord(id: string, updates: Partial<IncomeRecord>): Promise<IncomeRecord> {
    ...
    return await firestore.runTransaction(async (transaction) => {
      ...
      // Guard against modifying financial core fields of income records generated by contribution payment requests
      if (oldRecord.contributionPaymentRequestId) {
        if (updates.amount !== undefined && updates.amount !== oldRecord.amount) {
          throw new Error('Cannot modify the amount of an approved member contribution income record.');
        }
        if (updates.accountId !== undefined && updates.accountId !== oldRecord.accountId) {
          throw new Error('Cannot change the receiving bank account of an approved member contribution income record.');
        }
        if (updates.referenceNumber !== undefined && updates.referenceNumber !== oldRecord.referenceNumber) {
          throw new Error('Cannot change the bank reference number of an approved member contribution income record.');
        }
      }
  ...
  // Automatically generate and synchronize fee and fine records for club members
  // From membershipFeeStartDate until deactivationDate (or current month if active)
  async ensureMemberContributions(targetMemberId?: string): Promise<{ generated: number; updated: number }> {
    try {
      const settings = await this.getContributionSettings();
      let members = await this.getMembers();
      if (targetMemberId) {
        members = members.filter(m => m.id === targetMemberId);
      }
      ...
      const enableAutoFines = Boolean(settings.enableAutoFines);

      let generated = 0;
      let updated = 0;

      for (const member of members) {
        // 1. Determine member fee start date
        const rawStartDate = member.membershipFeeStartDate || member.joinedDate || '2024-01-01';
        const [rawSY, rawSM] = String(rawStartDate).split('-');
        let startYear = Number(rawSY);
        let startMonth = Number(rawSM);
        if (isNaN(startYear) || startYear < 2000) startYear = 2024;
        if (isNaN(startMonth) || startMonth < 1 || startMonth > 12) startMonth = 1;

        // 2. Determine member fee end date (stops if deactivated by select date)
        let endYear = currentYear;
        let endMonth = currentMonth;
        if (member.status === 'inactive' && member.deactivationDate) {
          const [rawEY, rawEM] = String(member.deactivationDate).split('-');
          const ey = Number(rawEY);
          const em = Number(rawEM);
          if (!isNaN(ey) && ey >= 2000) endYear = ey;
          if (!isNaN(em) && em >= 1 && em <= 12) endMonth = em;
        }

        // Fetch existing contributions for this member
        const snap = await firestore.collection('memberContributions').where('memberId', '==', member.id).get();
        const existingDocs = snap.docs.map(d => ({ ref: d.ref, data: d.data() as MemberContributionRecord }));
        const existingMap = new Map<string, { ref: any; data: MemberContributionRecord }>();
        existingDocs.forEach(item => {
          existingMap.set(`${item.data.year}_${item.data.month}`, item);
        });

        let y = startYear;
        let m = startMonth;
        const validPeriodKeys = new Set<string>();

        while (y < endYear || (y === endYear && m <= endMonth)) {
          const periodKey = `${y}_${m}`;
          validPeriodKeys.add(periodKey);

          const docId = `contrib_${member.id}_${y}_${m}`;
          const docRef = firestore.collection('memberContributions').doc(docId);

          const graceDate = new Date(y, m - 1, dueDay + graceDays);
          let fineAmount = 0;
          let fineDays = 0;
          let isOverdue = false;

          if (now > graceDate) {
            fineDays = Math.max(0, Math.floor((now.getTime() - graceDate.getTime()) / (1000 * 3600 * 24)));
            if (enableAutoFines) {
              fineAmount = Math.min(fineDays * finePerDay, monthlyFee * 2);
            }
            isOverdue = true;
          }

          const existingItem = existingMap.get(periodKey);
          if (!existingItem) {
            const newRecord: MemberContributionRecord = {
              ...
              fineAmount,
              discountAmount: 0,
              totalPayable: monthlyFee + fineAmount,
              paidAmount: 0,
            ...
            if (existing.status !== 'paid' && existing.status !== 'waived') {
              ...
              const unpaidBase = Math.max(0, (existing.baseAmount || monthlyFee) - (existing.paidAmount || 0));
              const totalPayable = (existing.paidAmount || 0) + unpaidBase + fineAmount;

              if (
                existing.fineAmount !== fineAmount ||
                existing.fineDays !== fineDays ||
        ...
        // Cleanup: remove unpaid records outside the member's valid period
        for (const item of existingDocs) {
          const key = `${item.data.year}_${item.data.month}`;
          if (!validPeriodKeys.has(key) && item.data.status !== 'paid' && item.data.status !== 'waived') {
      ...
      console.warn('ensureMemberContributions error:', err);
      return { generated: 0, updated: 0 };
    }
  }

  // Calculate waterfall payment allocation across oldest unpaid months and fines, with balance carry forward
  async previewContributionWaterfall(
    memberId: string,
    paymentAmount: number,
    useCreditBalance: boolean = true
  ): Promise<{
    memberId: string;
    paymentAmount: number;
    existingCreditBalance: number;
    ...
    fullySettledMonthsCount: number;
  }> {
    await this.ensureMemberContributions(memberId);
    const member = await this.getMemberById(memberId);
    const settings = await this.getContributionSettings();
    const monthlyFee = Number(settings.monthlyFee || 50);

    const existingCreditBalance = Number(member?.creditBalance || 0);
    const totalAvailable = Number(paymentAmount || 0) + (useCreditBalance ? existingCreditBalance : 0);

    const contribs = await this.getMemberContributions({ memberId });
    const unpaid = contribs.filter(c => c.status !== 'paid' && c.status !== 'waived');
    unpaid.sort((a, b) => (a.year * 12 + a.month) - (b.year * 12 + b.month));

    let remaining = totalAvailable;
    let totalFinesPaid = 0;
    let totalBasePaid = 0;
    const allocations: WaterfallAllocationItem[] = [];

    for (const rec of unpaid) {
      if (remaining <= 0) break;
      const fineDue = Number(rec.fineAmount || 0);
      const baseDue = Math.max(0, Number(rec.baseAmount || monthlyFee) - Number(rec.paidAmount || 0));

      // 1. Auto slip paying amount first unpaid month and fine
      const finePaid = Math.min(remaining, fineDue);
      remaining -= finePaid;
      totalFinesPaid += finePaid;

      // 2. Pay base fee next
      const basePaid = Math.min(remaining, baseDue);
      remaining -= basePaid;
      totalBasePaid += basePaid;

      const totalPaidForMonth = finePaid + basePaid;
      const isFullyPaid = (finePaid === fineDue) && (basePaid === baseDue);
      const remainingDue = (fineDue - finePaid) + (baseDue - basePaid);

      allocations.push({
        year: rec.year,
        month: rec.month,
        monthName: MONTH_NAMES[rec.month - 1] || `Month ${rec.month}`,
        docId: rec.id,
        baseAmount: rec.baseAmount || monthlyFee,
        fineAmount: fineDue,
        finePaid,
        basePaid,
        totalPaid: (rec.paidAmount || 0) + totalPaidForMonth,
        status: isFullyPaid ? 'paid' : 'partial',
        remainingDue
      });
    }

    const carryForwardBalance = Math.max(0, remaining);
    const fullySettledMonthsCount = allocations.filter(a => a.status === 'paid').length;
    ...
    return {
      ...
      totalFinesPaid,
      totalBasePaid,
      totalPaid: totalAvailable - remaining,
      carryForwardBalance,
      newCreditBalance: carryForwardBalance,
      fullySettledMonthsCount
    };
  }
  ...
  async getMemberContributions(filter?: { year?: number; month?: number; memberId?: string; status?: string }): Promise<MemberContributionRecord[]> {
    if (filter?.memberId) {
      await this.ensureMemberContributions(filter.memberId);
    }
  ...
  async getContributionPaymentRequestsSummary(): Promise<{
    pending: number;
    ...
    const snap = await firestore.collection('contributionPaymentRequests').get();
    const list = snap.docs.map(d => d.data() as ContributionPaymentRequest);
    ...
    for (const r of list) {
      ...
      if (r.submittedAt) {
        ...
        if (r.submittedAt.startsWith(todayStr) || subDate.toDateString() === now.toDateString()) {
          submittedToday++;
        }
      }

      if (r.status === 'approved') {
        const revTime = r.reviewedAt || r.updatedAt || r.submittedAt;
        if (revTime) {
          const revDate = new Date(revTime);
          if (revDate.getFullYear() === curYear && revDate.getMonth() === curMonth) {
            approvedThisMonth++;
          }
        }
      }
  ...
  async createContributionPaymentRequest(data: Partial<ContributionPaymentRequest>): Promise<ContributionPaymentRequest> {
    ...
    const requestNumber = data.requestNumber || await this.getNextRequestNumber('contributionPaymentRequests', 'CPR-', 5);
    const now = new Date().toISOString();
    ...
    const item: ContributionPaymentRequest = {
      ...
      memberNumber: data.memberNumber || '',
      memberName: data.memberName || '',
      contributionYear: data.contributionYear || data.year || currentYear,
      year: data.year || data.contributionYear || currentYear,
      paymentType: (data.paymentType as any) ?? null,
      ...
      calculatedAmount: data.calculatedAmount ?? data.totalAmount ?? null,
      ...
      normalizedReference: data.normalizedReference || null,
      referenceLock: data.referenceLock || null,
      slipStoragePath: data.slipStoragePath || '',
      slipDownloadUrl: data.slipDownloadUrl || '',
      slipDataUrl: data.slipDataUrl || (data.slipDownloadUrl?.startsWith('data:') ? data.slipDownloadUrl : ''),
      ...
      reviewedByName: null,
      incomeRecordId: null,
      contributionRecordIds: data.contributionRecordIds || [],
      createdAt: now,
      updatedAt: now
    };
  ...
  async updateContributionPaymentRequestSlip(
    id: string,
    slipData: {
    ...
    const current = snap.data() as ContributionPaymentRequest;
    const now = new Date().toISOString();
    const updates: Partial<ContributionPaymentRequest> = {
      slipDownloadUrl: slipData.slipDownloadUrl,
      updatedAt: now
    };
    if (slipData.slipDataUrl !== undefined) updates.slipDataUrl = slipData.slipDataUrl;
    if (slipData.slipStoragePath !== undefined) updates.slipStoragePath = slipData.slipStoragePath;
    if (slipData.slipFileName !== undefined) updates.slipFileName = slipData.slipFileName;
    if (slipData.slipMimeType !== undefined) updates.slipMimeType = slipData.slipMimeType;
    if (slipData.slipFileSize !== undefined) updates.slipFileSize = slipData.slipFileSize;
    if (slipData.referenceNumber !== undefined && slipData.referenceNumber.trim()) {
      updates.referenceNumber = slipData.referenceNumber.trim();
    }
  ...
  async approveContributionPaymentRequest(
    ...
    options?: {
      referenceNumber?: string;
      paymentType?: 'monthly' | 'annual' | 'single_month' | 'multiple_months';
      month?: number;
      accountId?: string;
      approvalNote?: string;
    ...
    const opts = typeof options === 'string' ? { approvalNote: options } : (options || {});
    const rawRef = String(opts.referenceNumber || '').trim();
    if (!rawRef) {
      throw new Error('Bank Reference Number is mandatory for approving contribution payment.');
    }
    const normalizedRef = rawRef.toUpperCase().replace(/[^A-Z0-9]/g, '');
    if (!normalizedRef) {
      throw new Error('Invalid Bank Reference Number format.');
    }
    const refLockId = `reflock_${normalizedRef}`;
    ...
    const targetYear = Number(reqInitial.contributionYear || reqInitial.year || new Date().getFullYear());

    // Resolve target bank account
    const bankAccounts = await this.getBankAccounts();
    const targetAccountId = opts.accountId || reqInitial.accountId || settings.defaultDepositAccountId || bankAccounts.find(a => a.status === 'active')?.id;
    if (!targetAccountId) {
      throw new Error('Receiving bank account is required for payment approval.');
    }
    ...
    let resolvedPaymentType: 'monthly' | 'annual' | 'waterfall' = 'waterfall';
    let waterfallResult: any = null;

    if (opts.paymentType === 'annual' || reqInitial.paymentType === 'annual') {
      resolvedPaymentType = 'annual';
      // Annual advance payment covering all 12 months with discount
      const annualBase = monthlyFee * 12;
      const discount = annualDiscountMonths * monthlyFee;
      totalCalculated = Math.max(0, annualBase - discount);
      totalFinesDeducted = 0;
      totalDiscountApplied = discount;
      coveredMonths = Array.from({ length: 12 }, (_, i) => i + 1);

      const monthlyDiscountShare = Math.round((discount / 12) * 100) / 100;
      const monthlyPayable = Math.round((totalCalculated / 12) * 100) / 100;
      ...
      for (let m = 1; m <= 12; m++) {
        ...
        allocations.push({
          ...
          baseAmount: monthlyFee,
          fineAmount: 0,
          discountAmount: m === 12 ? (discount - (monthlyDiscountShare * 11)) : monthlyDiscountShare,
      ...
      // Automatic Waterfall Payment Logic:
      // Pays oldest unpaid month & fines first, reming to current/paid month, and carry forward balance
      const rawPayment = Number(opts.approvedAmount || (opts as any).amountPaid || reqInitial.amountPaid || reqInitial.totalAmount || reqInitial.calculatedAmount || monthlyFee);
      const paymentAmount = rawPayment > 0 ? rawPayment : monthlyFee;

      waterfallResult = await this.previewContributionWaterfall(reqInitial.memberId, paymentAmount, true);

      totalCalculated = paymentAmount;
      totalFinesDeducted = waterfallResult.totalFinesPaid;
      totalDiscountApplied = 0;
      coveredMonths = waterfallResult.allocations.map((a: any) => a.month);
      resolvedPaymentType = 'waterfall';

      for (const item of waterfallResult.allocations) {
        allocations.push({
          year: item.year,
          month: item.month,
          docId: item.docId,
          ...
          discountAmount: 0,
          totalPayable: item.baseAmount + item.fineAmount,
    ...
    return await firestore.runTransaction(async (transaction) => {
      ...
      // Check Reference Lock to prevent double-spending / duplicate approvals
      const refLockRef = firestore.collection('paymentReferenceLocks').doc(refLockId);
      const refLockSnap = await transaction.get(refLockRef);
      if (refLockSnap.exists) {
        throw new Error('This bank reference number has already been used.');
      }

      // Read Receiving Account
      const accountRef = firestore.collection('budgetAccounts').doc(targetAccountId);
      const accountSnap = await transaction.get(accountRef);
      if (!accountSnap.exists) {
        throw new Error(`Receiving bank account ${targetAccountId} not found.`);
      }
      const targetAccount = accountSnap.data() as BankAccount;
      if (targetAccount.status !== 'active') {
        throw new Error(`Selected receiving account (${targetAccount.accountName}) is inactive.`);
      }

      // Read Member document
      const memberRef = firestore.collection('clubMembers').doc(request.memberId);
      const memberSnap = await transaction.get(memberRef);

      // Read Income Record Ref
      ...
      // Read all candidate contribution documents
      ...
      // 2. ALL WRITES AFTER READS
      const now = new Date().toISOString();
      const periodLabel = resolvedPaymentType === 'annual'
        ? `${targetYear} Annual Package (12 Months)`
        : allocations.length > 0
          ? `Auto-Split (${allocations.map(a => `${a.year}/${a.month}`).join(', ')})${waterfallResult?.carryForwardBalance ? ` [Carry Forward: ${waterfallResult.carryForwardBalance} MVR]` : ''}`
          : `Advance Contribution Deposit (${totalCalculated} MVR)`;

      const formattedTitle = resolvedPaymentType === 'annual'
        ? `Annual Membership Fee - ${request.memberName} - ${targetYear}`
        : `Membership Fee Payment - ${request.memberName} (${totalCalculated} MVR)`;

      // Write Reference Lock
      ...
      const incomeRecord: IncomeRecord = {
        ...
        notes: `Approved member contribution (${request.requestNumber}). Ref: ${rawRef}. Allocation: ${periodLabel}. ${opts.approvalNote || ''}`.trim(),
        status: 'received',
      ...
      const currentBalance = targetAccount.currentBalance || 0;
      transaction.update(accountRef, {
        currentBalance: currentBalance + totalCalculated,
        updatedAt: now
      });

      // Update Member Credit Balance if carry forward exists or member exists
      if (memberSnap.exists && waterfallResult) {
        transaction.update(memberRef, {
          creditBalance: waterfallResult.carryForwardBalance,
          updatedAt: now
        });
      }

      // Write Covered Member Contributions
      ...
      const updatedRequest: ContributionPaymentRequest = {
        ...
        month: allocations[0]?.month ?? null,
        months: coveredMonths,
        contributionYear: targetYear,
        year: targetYear,
        totalAmount: totalCalculated,
        calculatedAmount: totalCalculated,
        amountPaid: totalCalculated,
        baseAmount: (resolvedPaymentType as string) === 'monthly' ? monthlyFee : (monthlyFee * coveredMonths.length),
        fineAmount: totalFinesDeducted,
        discountAmount: totalDiscountApplied,
        splitAllocations: waterfallResult ? waterfallResult.allocations : undefined,
        carryForwardBalance: waterfallResult ? waterfallResult.carryForwardBalance : 0,
        referenceNumber: rawRef,
        normalizedReference: normalizedRef,
        referenceLock: refLockId,
        accountId: targetAccount.id,
        accountName: targetAccount.accountName,
        accountNumber: targetAccount.accountNumber || '',
        bankName: targetAccount.bankName || 'Bank of Maldives',
        reviewedAt: now,
      ...
      return { request: updatedRequest, incomeRecord, waterfall: waterfallResult };
    });
  }
  ...
  async batchGenerateContributions(year: number, month: number): Promise<{ generated: number; skipped: number }> {
    ...
    for (const m of members) {
      // 1. Check fee start date
      const rawStartDate = m.membershipFeeStartDate || m.joinedDate || '2024-01-01';
      const [syStr, smStr] = rawStartDate.split('-');
      const sy = Number(syStr) || 2024;
      const sm = Number(smStr) || 1;
      const startVal = sy * 12 + sm;

      if (periodVal < startVal) {
        skipped++;
        continue;
      }

      // 2. Check deactivation date
      if (m.status === 'inactive' && m.deactivationDate) {
        const [eyStr, emStr] = m.deactivationDate.split('-');
      ...
      if (existing.exists) {
        ...
        const dueDay = settings.dueDayOfMonth || 10;
        const graceDays = settings.gracePeriodDays ?? 5;
        const graceDate = new Date(year, month - 1, dueDay + graceDays);
        let fineAmount = 0;
        let fineDays = 0;
        let isOverdue = false;
        ...
        if (now > graceDate) {
          ...
          if (settings.enableAutoFines) {
            fineAmount = Math.min(fineDays * (settings.finePerDay || 5), (settings.monthlyFee || 50) * 2);
          }
          isOverdue = true;
        }
        ...
        const record: MemberContributionRecord = {
          ...
          fineAmount,
          discountAmount: 0,
          totalPayable: (settings.monthlyFee || 50) + fineAmount,
          paidAmount: 0,
